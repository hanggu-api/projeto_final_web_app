...
void main() {
  patrolTest(
    'Monitor Real Notifications and Auto-Open Flow',
    ($) async {
      ...
      try {
        await $.native.tap(Selector(text: 'Limpar tudo'));
      } catch (e) {
        // Might not have notifications, ignore
      }
      await $.native.pressBack();

      print('🚀 [PATROL] Monitorando notificações reais...');
      print('💡 [PATROL] Por favor, dispare um pedido real agora.');

      // 3. Loop to wait and detect notifications
      bool notificationFound = false;
      for (int i = 0; i < 60; i++) { // Wait up to 5 minutes (60 * 5s)
        ...
        try {
           await $.native.tap(Selector(textContains: 'Novo Serviço'));
           print('✅ [PATROL] Notificação detectada e clicada!');
      ...
      print('🎉 [PATROL] Sucesso: Modal de serviço detectado após notificação!');
    },
  );
}
