...
class NavigationHelper {
  ...
  static Future<bool> openNavigation({
    ...
    final googleFallbackUri = Uri.parse('https://www.google.com/maps/dir/?api=1&destination=$lat,$lon&travelmode=driving');

    try {
      if (!kIsWeb) {
        if (Platform.isAndroid) {
          if (await canLaunchUrl(androidUri)) {
            return await launchUrl(androidUri, mode: LaunchMode.externalApplication);
          }
      ...
      return await launchUrl(googleFallbackUri, mode: LaunchMode.externalApplication);
    } catch (e) {
