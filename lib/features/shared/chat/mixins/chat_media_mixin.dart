...
mixin ChatMediaMixin<T extends StatefulWidget> on State<T>, ChatStateMixin<T> {
  ...
  Future<String?> uploadToStorage({
    ...
    try {
      if (kIsWeb) {
        if (bytes != null) {
          return await api.uploadToCloud(
            bytes,
            filename: filename,
            serviceId: serviceId,
            type: 'chat',
        ...
        if (path != null) {
          return await api.uploadMediaFromPath(
            path,
            filename: filename,
            serviceId: serviceId,
            type: 'chat',
  ...
  Future<void> openUnifiedCamera(
    String serviceId,
    VoidCallback onScroll, {
    bool initialVideoMode = false,
  }) async {
    try {
      final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => InAppCameraScreen(initialVideoMode: initialVideoMode),
        ),
      );
      ...
      if (isVideo) {
        ...
        await processImageUpload(
          XFile(path, bytes: bytes, name: name),
          serviceId: serviceId,
          onScroll: onScroll,
        );
      }
  ...
  Future<void> processImageUpload(
    XFile f, {
    required String serviceId,
    required VoidCallback onScroll,
  }) async {
    ...
    try {
      if (!isRecording) {
        ...
        setState(() {
          ...
          recordTicker?.cancel();
          recordTicker = Timer.periodic(const Duration(seconds: 1), (_) {
            if (mounted) setState(() {});
          });
        });
      } else {
