...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _loadProfile() async {
    ...
    try { await _api.getProfile(); } catch (_) {}
  }
  ...
  Future<void> _recoverActiveTrip() async {
    if (_api.userId == null) return;
    try {
      final trip = await UberService().getActiveTripForClient(_api.userId!);
      if (trip != null && mounted) {
        debugPrint('✅ [Reconexão] Viagem ativa encontrada: ${trip['id']} (${trip['status']}). Redirecionando para acompanhamento...');
        // O app original gerencia a viagem num tela separada (UberTrackingScreen) e não preenchendo as vars isoladas da home.
        context.push('/uber-tracking/${trip['id']}');
      }
    } catch (e) {
  ...
  void _initRefreshTimer() {
    refreshTimer = Timer.periodic(const Duration(seconds: 30), (_) => _loadServices());
  }
      ...
      onPopInvokedWithResult: (didPop, result) {
        ...
        if (isInTripMode && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Conclua ou cancele a viagem antes de sair.')),
          );
        }
      },
      child: Scaffold(
          ...
          children: [
            ...
            Positioned.fill(
              child: HomeMapWidget(
                ...
                onPickingLocationChanged: (pos) => setState(() => pickedLocation = pos),
  ...
  Widget _buildHeader() {
    ...
    return SafeStitchHeader(unreadCount: unreadCountCount, bellController: bellController);
  }
  ...
  Widget _buildFloatingControls() {
    return Positioned(
      ...
      child: AnimatedOpacity(
        ...
        child: Column(
          children: [
            _buildZoomButton(Icons.add, () => mapController.move(mapController.camera.center, mapController.camera.zoom + 1)),
            const SizedBox(height: 8),
            _buildZoomButton(Icons.remove, () => mapController.move(mapController.camera.center, mapController.camera.zoom - 1)),
  ...
  Widget _buildLocationButton() {
    return GestureDetector(
      ...
      child: Container(
        width: 50, height: 50,
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: AppTheme.primaryBlue.withOpacity(0.2), blurRadius: 20)]),
  ...
  Widget _buildBottomPanel(double offset) {
    if (isInTripMode) {
      // Aqui entrarão os widgets específicos de viagem passados pelo usuário no request
      return const Align(alignment: Alignment.bottomCenter, child: Text('Painel de Viagem Ativo')); 
    }
...
class SafeStitchHeader extends StatelessWidget {
  ...
  const SafeStitchHeader({super.key, required this.unreadCount, required this.bellController});
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            child: const Row(children: [Icon(Icons.bolt, color: Colors.amber, size: 20), SizedBox(width: 8), Text('101 SERVICE', style: TextStyle(fontWeight: FontWeight.bold))]),
          ),
          _buildCircleButton(Icons.notifications_none, () {}, badgeCount: unreadCount),
        ],
      ),
    );
  }

  Widget _buildCircleButton(IconData icon, VoidCallback onTap, {int badgeCount = 0}) {
    return GestureDetector(
      onTap: onTap,
      child: Stack(
        children: [
          Container(width: 44, height: 44, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: Icon(icon, color: AppTheme.textDark)),
          if (badgeCount > 0) Positioned(right: 0, top: 0, child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle), child: Text(badgeCount.toString(), style: const TextStyle(color: Colors.white, fontSize: 10)))),
        ],
      ),
    );
  }
