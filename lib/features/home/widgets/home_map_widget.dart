...
class _HomeMapWidgetState extends State<HomeMapWidget> 
  ...
  final List<AnimationController> _moveControllers = []; // Rastreia animações de movimento
  ...
  bool _isHeadingUp = true; // Inicia com rotação automática
  ...
  void _startHeadingTracking() {
    _positionStream = Geolocator.getPositionStream(
      ...
      final newRotation = CompassService.normalizeHeading(_mapRotation, targetRotation);
  ...
  void didUpdateWidget(HomeMapWidget oldWidget) {
    ...
    if (widget.driverLatLng != null && oldWidget.driverLatLng != null && 
        widget.driverLatLng != oldWidget.driverLatLng) {
      final bearing = _calculateBearing(oldWidget.driverLatLng!, widget.driverLatLng!);
      if (bearing != 0) {
        setState(() {
    ...
    if (widget.driverLatLng != null && oldWidget.driverLatLng == null && _mapReady) {
    ...
    if (widget.routePolyline.isNotEmpty && 
        (oldWidget.routePolyline.isEmpty || widget.routePolyline.length != oldWidget.routePolyline.length) &&
        _mapReady) {
  ...
  void _animatedMapMove(LatLng destLocation, double destZoom, Duration duration) {
    if (!mounted) return;
  ...
  void _centerRouteWithAnimation() {
    ...
    final bool isEnRoute = widget.tripStatus == 'accepted' || widget.tripStatus == 'arrived';
    
    if (isEnRoute && widget.driverLatLng != null) {
      minLat = min(widget.driverLatLng!.latitude, widget.routePolyline[0].latitude);
      maxLat = max(widget.driverLatLng!.latitude, widget.routePolyline[0].latitude);
      minLng = min(widget.driverLatLng!.longitude, widget.routePolyline[0].longitude);
      maxLng = max(widget.driverLatLng!.longitude, widget.routePolyline[0].longitude);
  ...
  double _calculateFitZoom(LatLngBounds bounds, Size screenSize, EdgeInsets padding) {
    ...
    final lngZoom = log(360 / lngDelta) / log(2) + log(cos(southWest.latitude * pi / 180).abs()) / log(2);
        ...
        onMapReady: () {
          if (mounted) {
            ...
            if (widget.routePolyline.isNotEmpty && !_animationPlayed) {
              ...
              Future.delayed(const Duration(milliseconds: 500), _playRouteAnimation);
            }
          }
        },
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: AppTheme.primaryYellow.withOpacity(0.5 * (1.0 - _pulseController.value)),
                        ...
                        Container(
                          ...
                          child: const Center(
                            child: Icon(Icons.person, color: AppTheme.textDark, size: 12),
                          ),
                        ),
                      ],
                    );
                    ...
                    Positioned(
                      ...
                      child: Row(
                        ...
                        children: List.generate(3, (index) => Icon(
                          Icons.chevron_right,
                          color: Colors.green.withOpacity(0.5 - (index * 0.15)),
                          size: 16,
                        )),
                      ),
                    ),
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Row(
                        ...
                        children: List.generate(3, (index) => Icon(
                          Icons.chevron_right,
                          color: Colors.orange.withOpacity(0.2 + (index * 0.15)),
                          size: 16,
                        )),
                      ),
                    ),
