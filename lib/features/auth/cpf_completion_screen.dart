...
class _CpfCompletionScreenState extends State<CpfCompletionScreen> {
  ...
  Future<void> _loadExistingProfileData() async {
    ...
    final cpf = (row['document_value'] ?? '').toString();
    final birthDate = (row['birth_date'] ?? '').toString();
    final phone = (row['phone'] ?? row['mobile_phone'] ?? '').toString();

    DateTime? parsedBirthDate;
    if (birthDate.isNotEmpty) {
      parsedBirthDate = DateTime.tryParse(birthDate);
    }

    setState(() {
      if (cpf.isNotEmpty) {
        _cpfController.text = _CpfInputFormatter().formatDigits(cpf);
      }
      if (phone.isNotEmpty) {
        _phoneController.text = formatPhoneDisplay(phone);
      }
      if (parsedBirthDate != null) {
        _selectedDate = parsedBirthDate;
        _birthDateController.text =
            '${parsedBirthDate.day.toString().padLeft(2, '0')}/'
            '${parsedBirthDate.month.toString().padLeft(2, '0')}/'
  ...
  Future<void> _submit() async {
    ...
    if (_verificationData['liveness_validated'] != true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Conclua a prova de vida para continuar.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      await _api.updateProfile(
        ...
        customFields: <String, dynamic>{
          ...
          if ((_verificationData['selfie_path'] ?? '').toString().isNotEmpty)
            'face_image_url': _verificationData['selfie_path'].toString(),
        },
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: _verificationData['liveness_validated'] != true
            ? FacialLivenessStep(
